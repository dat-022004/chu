...
bai_tap = os.path.join(THU_MUC, 'bai_tap.pdf')
TEN_PFX = os.path.join(THU_MUC, 'cert.pfx')
MAT_KHAU_PFX = b'1234'

# Kiểm tra file có tồn tại không
if not os.path.exists(bai_tap):
    raise FileNotFoundError(f"Không tìm thấy file PDF gốc: {bai_tap}")
if not os.path.exists(TEN_PFX):
    raise FileNotFoundError(f"Không tìm thấy file chứng chỉ: {TEN_PFX}")

# Đọc file gốc
with open(bai_tap, 'rb') as f:
...
# Tính tâm của vùng chữ ký để căn giữa tất cả
...
sign_phone_display = '0921224974'
oc.setFont(font_name, 10)

# Vẽ số điện thoại và ngày ký căn giữa (cách đều giữa "Sinh viên ký" và ảnh chữ ký)
oc.drawCentredString(text_center_x, img_y + 18 * mm, f'SĐT: {sign_phone_display}')
oc.drawCentredString(text_center_x, img_y + 14 * mm, f'Ngày ký: {sign_date_display}')
...
# Hiển thị họ tên đầy đủ ngay dưới ảnh chữ ký, căn giữa
oc.setFont(font_name, 10)
...
# Thử nhiều TSA server khác nhau (ưu tiên server hoạt động tốt)
tsa_servers = [
...
print("Đang ký PDF và thêm timestamp...")

for i, tsa_url in enumerate(tsa_servers):
    try:
        ...
        print(f"✓ Đã thêm timestamp thành công")
        break
    except Exception as e:
        # Chỉ hiển thị lỗi nếu thử hết tất cả server
        if i == len(tsa_servers) - 1:
            print(f"⚠ Không thể kết nối TSA server. Ký không có timestamp...")
        continue
...
try:
    ...
    if '/AcroForm' in test_reader.trailer['/Root']:
        ...
        if '/Fields' in acroform:
            sig_count = len(acroform['/Fields'])
    
    print(f"Người ký       : {dct['name']}")
    print(f"Số điện thoại  : {dct['contact']}")
    print(f"Địa điểm       : {dct['location']}")
    print(f"Lý do          : {dct['reason']}")
    print(f"Số trang       : {len(test_reader.pages)}")
    print(f"Chữ ký số      : {sig_count} chữ ký")
    print(f"File đầu ra    : {os.path.basename(TEN_DAU_RA)}")
    print("="*60 + "\n")
    
except Exception as e:
    print(f"✗ Lỗi kiểm tra PDF: {e}\n")
